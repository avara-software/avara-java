...
package com.avarasoftware.core

import com.avarasoftware.core.http.AsyncStreamResponse
import com.avarasoftware.core.http.Headers
import com.avarasoftware.core.http.HttpClient
import com.avarasoftware.core.http.LoggingHttpClient
import com.avarasoftware.core.http.PhantomReachableClosingHttpClient
import com.avarasoftware.core.http.QueryParams
import com.avarasoftware.core.http.RetryingHttpClient
...
private constructor(
     ...
     *
     * Use the one published in `avara-java-client-okhttp` or implement your own.
     *
     ...
     *
     * Defaults to [com.avarasoftware.core.jsonMapper]. The default is usually sufficient and rarely
     ...
     *
     * Defaults to the production environment: `https://api.avarasoftware.com`.
     */
    fun baseUrl(): String = baseUrl ?: PRODUCTION_URL

    /**
     * Webhook signing JWT secret for signature verification. Format: whsec_{base64}. Get this from
     * your Avara dashboard under API Keys > View JWT Secret.
     */
    ...
    companion object {
        ...
        const val PRODUCTION_URL = "https://api.avarasoftware.com"
    ...
    class Builder internal constructor() {
         ...
         * The Jackson JSON mapper to use for serializing and deserializing JSON.
         *
         * Defaults to [com.avarasoftware.core.jsonMapper]. The default is usually sufficient and
        ...
        /**
         * Webhook signing JWT secret for signature verification. Format: whsec_{base64}. Get this
         * from your Avara dashboard under API Keys > View JWT Secret.
         */
        ...
        fun fromEnv() = apply {
            ...
            (System.getProperty("avara.apiKey") ?: System.getenv("AVARA_API_KEY"))?.let {
                apiKey(it)
            }
            (System.getProperty("avara.webhookKey") ?: System.getenv("AVARA_WEBHOOK_KEY"))?.let {
                webhookKey(it)
            }
            System.getenv("AVARA_CUSTOM_HEADERS")?.let { customHeadersEnv ->
                            ...
                            object : ThreadFactory {
                                ...
                                override fun newThread(runnable: Runnable): Thread =
                                    threadFactory.newThread(runnable).also {
                                        it.name =
                                            "avara-stream-handler-thread-${count.getAndIncrement()}"
                                    }
                            }
                        )
                    )
