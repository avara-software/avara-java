...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { AvaraInvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw AvaraInvalidDataException("`$name` is not set")
            is JsonNull -> throw AvaraInvalidDataException("`$name` is null")
            else -> throw AvaraInvalidDataException("`$name` is invalid, received $this")
        }
