...
/**
 * Response for inviting a user to Viewer. Level is restricted to admin/member since owners cannot
 * be invited via API.
 */
...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(userInviteResponse: UserInviteResponse) = apply {
            canManageStudies = userInviteResponse.canManageStudies
            clinicRole = userInviteResponse.clinicRole
            createdAt = userInviteResponse.createdAt
            email = userInviteResponse.email
            firstName = userInviteResponse.firstName
            hasDashboardAccess = userInviteResponse.hasDashboardAccess
            invitedSource = userInviteResponse.invitedSource
            lastLoginAt = userInviteResponse.lastLoginAt
            lastName = userInviteResponse.lastName
            level = userInviteResponse.level
            userId = userInviteResponse.userId
            middleName = userInviteResponse.middleName
            phoneNumber = userInviteResponse.phoneNumber
            suffix1 = userInviteResponse.suffix1
            suffix2 = userInviteResponse.suffix2
    ...
    override fun toString() =
        "UserInviteResponse{canManageStudies=$canManageStudies, clinicRole=$clinicRole, createdAt=$createdAt, email=$email, firstName=$firstName, hasDashboardAccess=$hasDashboardAccess, invitedSource=$invitedSource, lastLoginAt=$lastLoginAt, lastName=$lastName, level=$level, userId=$userId, middleName=$middleName, phoneNumber=$phoneNumber, suffix1=$suffix1, suffix2=$suffix2, additionalProperties=$additionalProperties}"
}
